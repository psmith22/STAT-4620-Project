---
title: "STAT 4260 Project Random Forreest"
...
train$Lot.Frontage[is.na(train$Lot.Frontage)]<-as.integer(median(train$Lot.Frontage,na.rm=TRUE))
train$Garage.Yr.Blt[is.na(train$Garage.Yr.Blt)]<-as.integer(median(train$Garage.Yr.Blt,na.rm=TRUE))
train$Mas.Vnr.Area[is.na(train$Mas.Vnr.Area)]<-as.integer(median(train$Mas.Vnr.Area,na.rm=TRUE))
train$Bsmt.Full.Bath[is.na(train$Bsmt.Full.Bath)]<-as.integer(median(train$Bsmt.Full.Bath,na.rm=TRUE))
train$Bsmt.Half.Bath[is.na(train$Bsmt.Half.Bath)]<-as.integer(median(train$Bsmt.Half.Bath,na.rm=TRUE))
train$BsmtFin.SF.1[is.na(train$BsmtFin.SF.1)]<-as.integer(median(train$BsmtFin.SF.1,na.rm=TRUE))
train$BsmtFin.SF.2[is.na(train$BsmtFin.SF.2)]<-as.integer(median(train$BsmtFin.SF.2,na.rm=TRUE))
train$Bsmt.Unf.SF[is.na(train$Bsmt.Unf.SF)]<-as.integer(median(train$Bsmt.Unf.SF,na.rm=TRUE))
train$Total.Bsmt.SF[is.na(train$Total.Bsmt.SF)]<-as.integer(median(train$Total.Bsmt.SF,na.rm=TRUE))
train$Garage.Cars[is.na(train$Garage.Cars)]<-as.integer(median(train$Garage.Cars,na.rm=TRUE))
train$Garage.Area[is.na(train$Garage.Area)]<-as.integer(median(train$Garage.Area,na.rm=TRUE))
test$Lot.Frontage[is.na(test$Lot.Frontage)]<-as.integer(median(test$Lot.Frontage,na.rm=TRUE))
test$Garage.Yr.Blt[is.na(test$Garage.Yr.Blt)]<-as.integer(median(test$Garage.Yr.Blt,na.rm=TRUE))
test$Mas.Vnr.Area[is.na(test$Mas.Vnr.Area)]<-as.integer(median(test$Mas.Vnr.Area,na.rm=TRUE))
sapply(test, class)
sapply(train, class)
test$Bsmt.Full.Bath[is.na(test$Bsmt.Full.Bath)]<-as.integer(median(test$Bsmt.Full.Bath,na.rm=TRUE))
test$Bsmt.Half.Bath[is.na(test$Bsmt.Half.Bath)]<-as.integer(median(test$Bsmt.Half.Bath,na.rm=TRUE))
test$BsmtFin.SF.1[is.na(test$BsmtFin.SF.1)]<-as.integer(median(test$BsmtFin.SF.1,na.rm=TRUE))
test$BsmtFin.SF.2[is.na(test$BsmtFin.SF.2)]<-as.integer(median(test$BsmtFin.SF.2,na.rm=TRUE))
test$Bsmt.Unf.SF[is.na(test$Bsmt.Unf.SF)]<-as.integer(median(test$Bsmt.Unf.SF,na.rm=TRUE))
test$Total.Bsmt.SF[is.na(test$Total.Bsmt.SF)]<-as.integer(median(test$Total.Bsmt.SF,na.rm=TRUE))
test$Garage.Cars[is.na(test$Garage.Cars)]<-as.integer(median(test$Garage.Cars,na.rm=TRUE))
test$Garage.Area[is.na(test$Garage.Area)]<-as.integer(median(test$Garage.Area,na.rm=TRUE))
...
```{r}
# equalize classes of train and test
test <- rbind(train[1,], test)
test <- test[-1,]

# use cv to determine optimal mtry value
set.seed(1)
...
rf.mod <- train(SalePrice~., data = train, methond = 'rf', trControl = ctrl, 
                    tuneLength = 10, ntree = 500)
...
# train Random Forest
# use mtry=48 for regression tree
rf.ames = randomForest(SalePrice ~ ., data = train, mtry = 48, importance = TRUE)
rf.pred = predict(rf.ames, newdata = test)

# determine mse and rss
