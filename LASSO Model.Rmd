...
# load libraries needed for ridge regression
library(glmnet)

# fit a cross-validated model
y_train = train$SalePrice
x_train = data.matrix(select(train, -SalePrice))
...
plot(preds.train.lasso,residuals.train)



# evaluate model performance on test data
y_test = test$SalePrice
x_test = data.matrix(select(test, -SalePrice))
preds.lasso = predict(lasso.best, newx = x_test)
residuals = y_test - preds.lasso
residuals.squared = residuals^2
MSE = mean(residuals.squared)
RSS = sum(residuals.squared)


# plot fitted values vs residuals
plot(preds.lasso, residuals)


```

Appears that model works well overall but we get extraneous results when our predicted house prices are really high.


Further work we could do: refine what plots to use for model checking. 
Possible question for Dr. Pratola: What is the standard for plots to check regression assumptions with a large number of predictors? Do we still want to plot each predictor against the residuals?
